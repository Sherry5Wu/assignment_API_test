...
test_data_file = os.path.join(Config.get_data_path(), "test_user.yml")

# 1.2 read the file using tools
test_data_list = YamlReader(test_data_file).data_all()


@pytest.mark.parametrize("user", test_data_list)
def test_user(user):
    # initialise url, data
    url = ConfigYaml().get_config_url() + user["url"]
...
#    print(res)
    testcase_name = user["case_name"]
    case_scenario = user["case_scenario"]
    expected_code = user["expect_code"]
    code = res["code"]
